...
from .docker_utils import (
    ...
    get_container_status_with_exit_code,
)
    ...
    for key, value in labels.items():
        if key.startswith("traefik.http.routers.") and key.endswith(".rule"):
            ...
            for host_ in host_matches:
                ...
                if entrypoints:
                    entrypoint_list = [
                        ep.strip().lower() for ep in entrypoints.split(",")
                    ]
                    is_https_entrypoint = any(
                        any(
                            key in ep
                            for key in ("https", "443", "secure", "ssl", "tls")
                        )
                ...
                routes.append(
                    {"router": router_name, "url": url, "rule": value, "host": host_}
                )
...
def should_use_https(port_str, container_port, https_ports_list):
    return (
        ...
        or port_str == "443"
        or port_str.endswith("443")
        or port_str in https_ports_list
    )
...
def build_port_map(
    ...
    for container_port, host_port, protocol in published_ports:
        link_hostname = _get_link_hostname(
            public_hostname, host_ip, is_docker_host, request_hostname
        )
        link = create_port_link(
            host_port, https_ports_list, link_hostname, container_port
        )
    ...
    if custom_ports_list:
        link_hostname = _get_link_hostname(
            public_hostname, None, is_docker_host, request_hostname
        )
...
def extract_labels_data(labels, tags_enable):
    stack_name = labels.get("com.docker.compose.project", "") or labels.get(
    ...
    return {
        ...
        "port_range_grouping": (
            port_range_grouping.lower() if port_range_grouping else None
        ),
        "tags": tags,
    }


def get_or_check_update(
    cache_key, client, container_or_service, server_name, image_name, is_swarm
):
    ...
    if is_swarm:
        ...
        return update_checker.check_local_image_updates(
            client, container_or_service, server_name
        )


def process_swarm_service(
    service,
    tasks_by_service,
    client,
    server_name,
    ...
    try:
        ...
        update_available = get_or_check_update(
            cache_key, client, service, server_name, image_name, True
        )
    ...
    try:
        ...
        if labels_data["custom_ports_list"]:
            ...
            for port in labels_data["custom_ports_list"]:
                link = create_port_link(
                    port, labels_data["https_ports_list"], link_hostname
                )
                port_map.append(
                    {
                        "container_port": "",
        ...
        cache_key = update_checker.get_cache_key(
            server_name, container.name, image_name
        )
        update_available = get_or_check_update(
            cache_key, client, container, server_name, image_name, False
        )
        ...
        container_info = {
            ...
            "is_swarm_standalone": is_swarm_standalone,
        }

        return container_info
    except Exception as e:
        logger.warning(
            f"Error processing container {getattr(container, 'name', 'unknown')}: {e}"
        )
        return {
            "server": server_name,
...
def is_swarm_task_container(container):
    try:
        labels = container.attrs.get("Config", {}).get("Labels", {}) or {}
        if labels.get("com.docker.swarm.service.name"):
            return True
        if labels.get("com.docker.swarm.task") == "true":
            return True
        return False
    except Exception:
        return False
...
def process_single_host_data(
    ...
    try:
        ...
        try:
            info = client.info()
            is_swarm = (
                info.get("Swarm", {}).get("LocalNodeState", "").lower() == "active"
            )
        except Exception:
            is_swarm = False
            ...
            try:
                ...
                try:
                    ...
                    for container in containers:
                        if is_swarm_task_container(container):
                            continue

                        standalone_info = process_container(
                            container,
                            client,
                            server_name,
                            public_hostname,
                            ...
                            port_range_grouping_enabled,
                            request_hostname,
                            is_swarm_standalone=True,
                        )
                        container_data.append(standalone_info)
                except Exception as standalone_error:
                    logger.warning(
                        f"Standalone container listing failed on swarm host {server_name}: {standalone_error}"
                    )
        ...
        for container in containers:
            try:
                ...
                logger.warning(
                    f"Error processing container {getattr(container, 'name', 'unknown')} on {server_name}: {container_error}"
                )
                container_data.append(
                    {
                        "server": server_name,
...
def get_all_data():
    ...
    server_list_for_json = [
        {"name": s["name"], "status": s["status"], "order": s["order"], "url": s["url"]}
        for s in servers
    ]
            ...
            try:
                ...
                if host["status"] != "inactive":
                    try:
                        ...
                        info = client.info()
                        is_swarm = (
                            info.get("Swarm", {}).get("LocalNodeState", "").lower()
                            == "active"
                        )
                        if is_swarm:
                ...
                logger.error(
                    f"Timeout processing host {host['name']} after {HOST_PROCESSING_TIMEOUT}s"
                )
